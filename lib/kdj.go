...
// NewKDJ new Func
func NewKDJ(list []*models.Kline, period int) *KDJ {
...
func (e *KDJ) calculationKD(records []*models.Kline) (rsv, k, d []float64) {
	...
	for i := 0; i < length; i++ {
		...
		if len(periodLowArr) > 1 {
			...
			// d[i] = (k[i] + 2.0*d[i-1]) / 3
			k[i] = (2.0/3)*k[i-1] + 1.0/3*rsv[i] // 当日K值=2/3×前一日K值＋1/3×当日RSV
			d[i] = (2.0/3)*d[i-1] + 1.0/3*k[i]   // 当日D值=2/3×前一日D值＋1/3×当日K值
